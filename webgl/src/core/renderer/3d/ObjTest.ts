...
async function main() {
  ...
  console.log("read load-------------");
  // const objHref = 'https://webglfundamentals.org/webgl/resources/models/windmill/windmill.obj';
  const objHref = "res/models/windmill/windmill.obj";
  const response = await fetch(objHref);
  const text = await response.text();
  const obj = OBJParseHelper.parseOBJ(text);
  ...
  const materials = OBJParseHelper.parseMTL(matTexts.join('\n'));
  const textures = {
    ...
    Object.entries(material)
      ...
      .forEach(([key, filename]) => {
        ...
        if (!texture) {
          ...
          // console.log("texture-------", filename, textureHref);
          const textureHref = "res/models/windmill/"+filename;
