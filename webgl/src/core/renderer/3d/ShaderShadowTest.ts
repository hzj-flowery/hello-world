...
class ShadowLight_WebGl1 {
  ...
  */
  ...
  public render() {
    ...
    /**
     * 在上一次drawScene中，每一个点都是*光照的纹理矩阵*光照的视口矩阵*光照的投影矩阵，最后算出点的位置信息，将结果存在深度纹理中
     * 同样在本次drawScene中，需要将点从这张纹理中的uv取出来，那么就需要乘以光照视口矩阵的逆矩阵，先将其转换到世界空间坐标系下
     * 再乘以投影矩阵，
     * 在上一次drawscene中，将点转换到视口坐标系下，又乘以投影矩阵，投影矩阵是产生一个近大远小的左右，其实就是先将视口坐标系下的x和y进行不同程度的缩放
     */
    let textureMatrix = glMatrix.mat4.copy(null,texMatrix);
