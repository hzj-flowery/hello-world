...
class Spine extends SY.SpriteBase {
    ...
    animSkin(skin: skeleton_Skin, a: number) {
    ...
    private renderDrawables(node: skeleton_Node) {
        ...
        var v = GameMainCamera.instance.getCameraByUUid(syRender.CameraUUid.base3D).getInversModelMatrix();
        var m = this.modelMatrix;
        this._glMatrix.mat4.mul(newMV,v,m)
        this.shader.bindMatrixToShader(syGL.AttributeUniform.VMMatrix,newMV);
        var pMatix = GameMainCamera.instance.getCameraByUUid(syRender.CameraUUid.base3D).getProjectionMatrix();
    ...
    protected collectRenderData(time:number):void{
          ...
          this.animSkin(this.gltf.skins[0], Math.sin(time) * .5);
           ...
           for (const scene of this.gltf.scenes) {
            ...
            scene.root.traverse(this.renderDrawables);
        }
...
async function main() {
    ...
    const gltf = skeleton_Parse.parseGLTF(gl);
    function degToRad(deg) {
        return deg * Math.PI / 180;
    }

    const origMatrices: Map<skeleton_Node, Float32Array> = new Map();
    function animSkin(skin: skeleton_Skin, a: number) {
        for (let i = 0; i < skin.joints.length; ++i) {
            const joint = skin.joints[i];
            // if there is no matrix saved for this joint
            if (!origMatrices.has(joint)) {
                // save a matrix for joint
                origMatrices.set(joint, joint.transform.getMatrix());
            }
            // get the original matrix
    ...
    function render(time) {
        ...
        glMatrix.mat4.perspective(projection, fieldOfViewRadians, aspect, 1, 2000);
        ...
        glMatrix.mat4.lookAt(camera, cameraPosition, target, up);

        // Make a view matrix from the camera matrix.
        const view = glMatrix.mat4.create();
        glMatrix.mat4.invert(view, camera);
        ...
        function renderDrawables(node: skeleton_Node) {
            //渲染网格
            for (const drawable of node.mesh_Drawables) {
                drawable.render(node, projection, view, sharedUniforms);
            }
