...
function _unrollLoops(string) {
  ...
  function replace(match, index, begin, end, snippet) {
    let unroll = '';
    let parsedBegin:any = parseInt(begin);
    let parsedEnd:any = parseInt(end);
...
export default class ProgramLib {
  ...
  private _cache:any;
  private _highpSupported:boolean;
  ...
  getProgram(name, defines, errPrefix) {
    ...
    program = new Program(this._device, {
      vert,
      frag
    });
