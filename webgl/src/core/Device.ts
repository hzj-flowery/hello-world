 ...
 * 
 ...
 采样 选取像素中心点作为采样点，每三个顶点形成一个三角形，被三角形三条边包围的像素块则生成片元
 像素的中心点是否在三角形的内部，这个往往会出现走样，也就是锯齿，解决办法
 1:超级采样抗锯齿技术(Super Sample Anti-aliasing, SSAA):
 光栅化拿到的屏幕参照尺寸不是1:1，而是n:1,这将产生n倍的片元
 当把片元输送到帧缓冲时，则开始降分辨率
 2：多采样抗锯齿(Multisample Anti-aliasing,MSAA):
 光栅化拿到的屏幕的参照尺寸为1：1，采样点变多，不再是1个，而是n个，根据三角形的边包围像素块的采样点多少，来决定一个像素块到底显示什么颜色
 边缘颜色被稀释变得更加光滑，和之前一样，只会产生一个片元
