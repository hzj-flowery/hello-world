...
local CustomActivitySingleRechargeItemCell = class("CustomActivitySingleRechargeItemCell", ListViewCellBase)
local TextHelper = require("app.utils.TextHelper")

 CustomActivitySingleRechargeItemCell.LINE_ITEM_NUM = 1


 CustomActivitySingleRechargeItemCell.ITEM_GAP = 106--奖励道具之间的间隔
 CustomActivitySingleRechargeItemCell.ITEM_ADD_GAP = 132--+道具之间的间隔
 CustomActivitySingleRechargeItemCell.ITEM_OR_GAP = 132--可选奖励道具之间间隔

function CustomActivitySingleRechargeItemCell:ctor()
	...
	local resource = {
		file = Path.getCSB("CustomActivitySingleRechargeItemCell", "customactivity"),
		binding = {
...
function CustomActivitySingleRechargeItemCell:_onCreate()
...
function CustomActivitySingleRechargeItemCell:onClickBtn()
...
function CustomActivitySingleRechargeItemCell:_onItemClick(sender)
	local curSelectedPos = sender:getTag()
	logWarn("CustomActivitySingleRechargeItemCell:_onIconClicked  "..curSelectedPos)
	self:dispatchCustomCallback(curSelectedPos)
end


function CustomActivitySingleRechargeItemCell:_onTouchCallBack(sender,state)
	-----------防止拖动的时候触发点击
...
function CustomActivitySingleRechargeItemCell:onClickItem(sender)
	local tag = self:getTag()
end

function CustomActivitySingleRechargeItemCell:_updateRewards(actTaskUnitData)
    ...
    self._commonRewardListNode:setGaps(CustomActivitySingleRechargeItemCell.ITEM_GAP,CustomActivitySingleRechargeItemCell.ITEM_ADD_GAP ,
        CustomActivitySingleRechargeItemCell.ITEM_OR_GAP)
    self._commonRewardListNode:updateInfo(rewards,rewardTypes)
end

--创建富文本
function CustomActivitySingleRechargeItemCell:_createProgressRichText(richText)
...
function CustomActivitySingleRechargeItemCell:updateInfo(data)
	...
	--logWarn(tostring(reachReceiveCondition).." CustomActivitySingleRechargeItemCell "..tostring(canReceive))
...
function CustomActivitySingleRechargeItemCell:setCallBack(callback)
	if callback then
