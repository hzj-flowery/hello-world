...
--
local RecoveryHeroNode = class("RecoveryHeroNode")
local TypeConvertHelper = require("app.utils.TypeConvertHelper")
local RecoveryHelper = require("app.scene.view.recovery.RecoveryHelper")
local EffectGfxNode = require("app.effect.EffectGfxNode")
local AudioConst = require("app.const.AudioConst")

function RecoveryHeroNode:ctor(target, index, onClickAdd, onClickDelete)
	self._target = target
	self._index = index
...
function RecoveryHeroNode:_initUI()
...
function RecoveryHeroNode:updateInfo(heroId, limitLevel, limitRedLevel)
	self:_initUI()
	if heroId then
...
function RecoveryHeroNode:_onButtonAddClicked()
	if self._onClickAdd then
		self._onClickAdd()
	end
end

function RecoveryHeroNode:_onButtonCloseClicked()
	self:updateInfo(nil)
...
function RecoveryHeroNode:reset()
...
function RecoveryHeroNode:playFlyEffect(callback)
	self._textName:setVisible(false)
	self._buttonAdd:setVisible(false)
	self._buttonClose:setVisible(false)
...
function RecoveryHeroNode:_playMoving(callback)
