...
local HorseFragListCell = class("HorseFragListCell", ListViewCellBase)
local TypeConvertHelper = require("app.utils.TypeConvertHelper")
local UserDataHelper = require("app.utils.UserDataHelper")

function HorseFragListCell:ctor()
	local resource = {
		file = Path.getCSB("HorseFragListCell", "horse"),
		binding = {
			_button1 = {
	...
	HorseFragListCell.super.ctor(self, resource)
end

function HorseFragListCell:onCreate()
	local size = self._resourceNode:getContentSize()
	self:setContentSize(size.width, size.height)
end

function HorseFragListCell:update(data1, data2)
...
function HorseFragListCell:_onButtonClicked1() 
	self:dispatchCustomCallback(1)
end

function HorseFragListCell:_onButtonClicked2()
	self:dispatchCustomCallback(2)
end

return HorseFragListCell