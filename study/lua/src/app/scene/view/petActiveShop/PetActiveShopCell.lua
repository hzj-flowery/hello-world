--
-- Author: Liangxu
-- Date: 2018-6-6 14:33:58
-- 神兽商店cell
local ListViewCellBase = require("app.ui.ListViewCellBase")
local PetActiveShopCell = class("PetActiveShopCell", ListViewCellBase)
local TypeConvertHelper = require("app.utils.TypeConvertHelper")
local ShopActiveDataHelper = require("app.utils.data.ShopActiveDataHelper")
local UserDataHelper = require("app.utils.UserDataHelper")

function PetActiveShopCell:ctor()
	local resource = {
	...
	PetActiveShopCell.super.ctor(self, resource)
end

function PetActiveShopCell:onCreate()
	local size = self._resourceNode:getContentSize()
	self:setContentSize(size.width, size.height)
end

function PetActiveShopCell:update(goodId1, goodId2)
...
function PetActiveShopCell:_showFragmentNum(type, value, index)
	self["_nodeFragment"..index]:removeAllChildren()
...
function PetActiveShopCell:_onClickButton1()
	self:dispatchCustomCallback(1)
end

function PetActiveShopCell:_onClickButton2()
	self:dispatchCustomCallback(2)
end

return PetActiveShopCell