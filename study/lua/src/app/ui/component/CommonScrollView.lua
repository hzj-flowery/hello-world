...
local CommonScrollView = class("CommonScrollView")
...
--
function CommonScrollView:ctor()
	self._target = nil
	self._textTitle = nil
	self._btnClose = nil
	self._totalRange = 0
...
--
function CommonScrollView:_init()
	self._spawnCount = 0
...
--
function CommonScrollView:bind(target)
	self._target = target
    self:_init()
...
--
function CommonScrollView:unbind(target)
    cc.unsetmethods(target, EXPORTED_METHODS)
end

function CommonScrollView:_initTouchEvent( ... )
...
--
function CommonScrollView:setCallback(update, selected, scrollEvent,eventListener)
...
--
function CommonScrollView:setCustomCallback(callback)
	self._customCallback = callback
end

--
function CommonScrollView:setTemplate(template, itemWidth, itemHeight)
...
function CommonScrollView:updateCellList(cellNum, notScroll)
	--数量相同的刷新
...
function CommonScrollView:_getNewOffset( ... )
	-- body
...
--
function CommonScrollView:resize(size, notScroll)
	--已经初始化过
	if self._totalCount and self._totalCount > 0 then
		self:updateCellList(size, notScroll)
		return
	end
...
--
function CommonScrollView:_respawn()
	if self._direction == ccui.ListViewDirection.vertical then
...
--
function CommonScrollView:_getItemPositionYInView(item)
...
--
function CommonScrollView:_getItemPositionXInView(item)
	local worldPos = item:getParent():convertToWorldSpaceAR(cc.p(item:getPosition()))
    local viewPos = self._target:convertToNodeSpaceAR(cc.p(worldPos))
    return viewPos.x
end
...
--
function CommonScrollView:getStartEndIndex()
	local startIdx, endIdx = self:_getStartEndIndex()
	return startIdx, endIdx
end

function CommonScrollView:_onUpdate(f)
...
function CommonScrollView:clearAll()
...
--根据总数量
--计算每个cell应该所在位置--预先生成
function CommonScrollView:_updateCellPositions()
...
--创建并更新Template
function CommonScrollView:tableCellAtIndex(index)
...
function CommonScrollView:_updateContentSize()
	-- 计算滚动范围
...
--根据偏移量，得到cellIndex
function CommonScrollView:_indexFromOffset(offset)
	local index = nil
...
function CommonScrollView:_offsetFromIndex(index)
...
--刷新当前listView
function CommonScrollView:reloadData( ... )
	-- body
...
function CommonScrollView:_getStartEndIndex( ... )
	-- body
...
function CommonScrollView:playEnterEffect( endCallBack )
...
function CommonScrollView:scrollViewDidScroll()
	local countOfItems = self._totalCount
...
function CommonScrollView:updateCellAtIndex(idx)
    if idx == -1 then
        return
	end
	--logWarn("CommonScrollView:updateCellAtIndex ::: "..idx)
    local countOfItems = self._totalCount
...
function CommonScrollView:cellAtIndex(idx)
...
function CommonScrollView:_moveCellOutOfSight(cell)
	--cell:retain()
...
function CommonScrollView:_setIndexForCell(index, cell)
...
function CommonScrollView:_addCellIfNecessary(cell)
...
function CommonScrollView:dequeueCell()
    local cell = nil
    if #self._cellsFreed == 0 then
        cell = nil
    else
        cell =  self._cellsFreed[1]
		--logWarn("CommonScrollView:dequeueCell 拿出老的复用")
...
function CommonScrollView:setLocation(index,customOffset)
...
function CommonScrollView:setLocationByPos(pos)
	self._target:stopAutoScroll()--定位位置，这时候不能处于滚动状态
...
--只适配垂直滚动，反馈Item底部位置
function CommonScrollView:getItemBottomLocation(index)
	local offset = self:_offsetFromIndex(index)
...
--只适配垂直滚动
function CommonScrollView:isInVisibleRegion(index)
...
function CommonScrollView:getItems( ... )
	return self._target:getChildren()
end

function CommonScrollView:getItem( index )
	-- body
	index = index + 1
...
function CommonScrollView:getItemByTag(index)
	--[[
...
function CommonScrollView:setSpacing(spacing )
