...
@ccclass
export default class TreasureLimitDetailRefineNode extends ListViewCellBase{
    @property({
    ...
    _updateSubView(index) {
        ...
        var rLevel = this._treasureUnitData.getRefine_level();
        if (index == 2) {
        ...
        var attrInfo = TreasureDataHelper.getTreasureRefineAttrWithConfigAndRLevel(configInfo, rLevel);
    ...
    _createAddLevel(index) {
        ...
        var node = cc.instantiate(this.commonDesValue).getComponent(CommonDesValue);
        node.setFontSize(20);
        node.updateUI(Lang.get('treasure_limit_refine_level_max_add'), '+' + TreasureConst.getAddRefineLevelByLimit());
        node.node.setPosition(24, 20);
        widget.addChild(node.node);
        widget.setAnchorPoint(0,0);
        widget.setContentSize(cc.size(402, 30));
