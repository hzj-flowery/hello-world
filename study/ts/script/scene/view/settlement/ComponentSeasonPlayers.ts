...
import PanelSeasonPlayers from "./PanelSeasonPlayers";

const { ccclass, property } = cc._decorator;
@ccclass
export default class ComponentSeasonPlayers extends ComponentBase {
    ...
    private _createCsbNode():cc.Node {
        ...
        cc.resources.load(Path.getPrefab('PanelSeasonPlayers', 'settlement'), cc.Prefab, (err, res:cc.Prefab)=> {
            if (err != null || res == null || !node.isValid) {
                return;
            }
            let panel = cc.instantiate(res).getComponent(PanelSeasonPlayers);
