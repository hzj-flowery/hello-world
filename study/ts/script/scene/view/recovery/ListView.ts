...
export default class ListView extends cc.Component {
    ...
    private _touchCallback: Function = null;
    ...
    public setTemplate(templatePrefab: cc.Prefab) {
        this.itemTemplate = templatePrefab;
        this._items.clear();
        this.recycleAll(true);
    }

    public setCallback(updateCallback: Function, touchCallback?: Function) {
    ...
    public resize(dataLength: number) {
    ...
    public getItems(): cc.Node[] {
        return this.content.children;
    }

    public scrollToItem(itemIndex: number, timeSecond?: number): boolean {
        if (this._dataLength <= 0 || !this.scrollView) {
            return false;
        }
        const count = this._dataLength <= 0 ? 1 : this._dataLength;
        this.column = this.column || 1;
        if (this.horizontal) {
            const maxWidth = this.content.width;
            const columnWidth = maxWidth / (count / this.column);

            this.scrollView.scrollToOffset(cc.v2(-columnWidth * itemIndex, 0), timeSecond);
        } else {
            const maxHeight = this.content.height;
            const rowHeight = maxHeight / (count / this.column);
            let pH = this.content.parent.height;

            this.scrollView.scrollToOffset(cc.v2(0, rowHeight * itemIndex), timeSecond);
        }
    }
    ...
    public getVisibleElements(): number {
        let visibleCount = 0;
        // const count = this._dataSet ? (this._dataSet.length || 1) : 1;
        const count = this._dataLength <= 0 ? 1 : this._dataLength;
        if (this.horizontal) {
            const maxWidth = this.content.width;
            const columnWidth = maxWidth / (count / this.column);
    ...
    public notifyUpdate() {
        ...
        if (this.horizontal) {
            this.content.width = Math.ceil(this._dataLength / this.column) * (this._itemWidth + this.spacing.x)
                - this.spacing.x + this.margin.x + this.margin.width;
        } else {
            this.content.height = Math.ceil(this._dataLength / this.column) * (this._itemHeight + this.spacing.y)
    ...
    private _layoutVertical(child: cc.Node, posIndex: number) {
        ...
        child.setPosition(
            this.column > 1 ? this.margin.x + child.width * child.anchorX
                + (child.width + this.spacing.x) * column - this.content.width * this.content.anchorX :
                (child.anchorX - 0.5) * child.width,
            -this.margin.y - child.height * (1 - child.anchorY + row) - this.spacing.y * row);
        // console.log(posIndex, child.position.x, child.position.y);
    }
    ...
    private recycleAll(destroy:boolean = false) {
        for (const child in this._filledIds) {
            if (this._filledIds.hasOwnProperty(child)) {
                if(destroy) {
                    this._filledIds[child] && this._filledIds[child].destroy()
                }else {
                    this.recycleNode(child);
                }
            }
        }
        this._filledIds = {};
    ...
    private recycleNode(filledId) {
        if (this._touchCallback != null && this._filledIds[filledId] != null) {
            this._filledIds[filledId].off(cc.Node.EventType.TOUCH_END);
        }
        this._items.put(this._filledIds[filledId]);
        this._filledIds[filledId] = null;
    }
    ...
    private updateView(visibleRange: number[]) {
        for (let i = visibleRange[0]; i <= visibleRange[1]; i++) {
            ...
            this._filledIds[i] = child;
            if (this._updateCallback != null) {
                this._updateCallback(child, i);
            }
            if (this._touchCallback != null) {
                child.on(cc.Node.EventType.TOUCH_END, this._touchCallback.bind(this, child, i));
            }
    ...
    private getVisibleRange(): number[] {
        ...
        if (visible >= this._dataLength) {
            visible = this._dataLength - 1;
        }
    ...
    public init() {
        if (this.scrollView) {
            ...
            if (this.horizontal) {
                ...
                this.content.x = 0;
                this.content.y = this.content.parent.height * (1 - this.content.parent.anchorY);
            }
        } else {
