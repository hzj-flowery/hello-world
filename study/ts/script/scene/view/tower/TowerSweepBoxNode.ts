...
@ccclass
export default class TowerSweepBoxNode extends cc.Component {
    ...
    onLoad() {
        ...
        this.node.setContentSize(size.width, size.height + TowerSweepBoxNode.HEIGHT_FIX);
        this._textTitle.string = (this._title);
        this._nodeBG2.node.active = (true);
        var rewardsNode = new cc.Node();
        var itemWidgets = this._updateAwards(rewardsNode, this._rewards, TowerSweepBoxNode.ITEM_GAP);
