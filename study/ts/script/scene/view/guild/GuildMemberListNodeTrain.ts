...
import { G_ConfigLoader, G_Prompt, G_SignalManager, G_UserData } from '../../../init';
...
import GuildMemberListCellTrain from './GuildMemberListCellTrain';
...
@ccclass
export default class GuildMemberListNodeTrain extends ViewBase {
    ...
    _activeLabel: cc.Label = null;

    @property({
        type: cc.Label,
        visible: true
    })
    _passiveLabel: cc.Label = null;

    @property({
        type: cc.Label,
        visible: true
    })
    _activeTimes: cc.Label = null;

    @property({
        type: cc.Label,
        visible: true
    })
    _passiveTimes: cc.Label = null;
    _categorySortFlag: any = [];
    _signalGuildQueryMall;
    _signalKickSuccess;
    _signalRedPointUpdate;
    _signalGuildUserPositionChange;
    _signalPromoteSuccess;
    _signalMailOnSendMail;
    _signalInviteTimeOut;
    _guildMemberList: any;
    _lastestSortCategory: number = 0;

    onCreate() {
        // this._listItemSource.setTemplate(GuildMemberListCellTrain);
        ...
        this._activeLabel.string = (Lang.get('guild_active_times'));
        this._passiveLabel.string = (Lang.get('guild_passive_times'));
    ...
    onEnter() {
        ...
        this._signalInviteTimeOut = G_SignalManager.add(SignalConst.EVENT_TRAIN_INVITE_TIME_OUT, handler(this, this._onEventInviteTimeOut));
    ...
    _updateList() {
        this._guildMemberList = UserDataHelper.getGuildMemberListBySort(this._lastestSortCategory - 1, this._categorySortFlag[this._lastestSortCategory]);
    ...
    updateListCell() {
        for (let i = this.recordIndex; i < this._guildMemberList.length; i++) {
            if (this._guildMemberList[i]) {
                var resource = cc.resources.get("prefab/guild/GuildMemberListCellTrain");
                var node1 = cc.instantiate(resource) as cc.Node;
                let cell = node1.getComponent(GuildMemberListCellTrain) as GuildMemberListCellTrain;
    ...
    _updateMyTrainTimes() {
        var myMemberData = G_UserData.getGuild().getMyMemberData();
        var passive = myMemberData.getTrain_daily_acptcount();
        var active = myMemberData.getTrain_daily_count();
        this._activeTimes.string = (active);
        this._passiveTimes.string = (passive);
    }
    ...
    onButtonDeclaration(sender) {
        var lv = G_ConfigLoader.getConfig(ConfigNameConst.PARAMETER).get(ParameterIDConst.GUILD_DECLARATION_LV);
    ...
    onButtonTitle7() {
        if (this._categorySortFlag[7] == null) {
            this._categorySortFlag[7] = false;
        } else {
            this._categorySortFlag[7] = !this._categorySortFlag[7];
        }
        this._lastestSortCategory = 7;
        this._refreshOrderArrow();
        this._updateList();
    }
    _refreshOrderArrow() {
    ...
    _refreshBtnState() {
        var haveCheckApplyPermission = UserDataHelper.isHaveGuildPermission(GuildConst.GUILD_JURISDICTION_6);
        var canSendMail = UserDataHelper.isHaveGuildPermission(GuildConst.GUILD_JURISDICTION_11);
        var canSetAnnouncement = UserDataHelper.isHaveGuildPermission(GuildConst.GUILD_JURISDICTION_7);
        var canSetDeclaration = UserDataHelper.isHaveGuildPermission(GuildConst.GUILD_JURISDICTION_8);
        var canModifyGuildName = UserDataHelper.isHaveGuildPermission(GuildConst.GUILD_JURISDICTION_10);
        this._btnApplyList.setVisible(haveCheckApplyPermission);
        this._btnDeclaration.setVisible(canSetDeclaration);
        this._btnSendMail.setVisible(canSendMail);
    }
