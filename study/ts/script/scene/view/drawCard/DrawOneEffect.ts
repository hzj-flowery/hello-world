...
@ccclass
export default class DrawOneEffect extends PopupBase {
    ...
    initData(awards: any, type?: any) {
        ...
        if (type == DrawOneEffect.DRAW_TYPE_MONEY) {
        ...
        } else if (type == DrawOneEffect.DRAW_TYPE_GOLD) {
    ...
    onFinishTouch(sender, event) {
        ...
        if (this._cardToOpen.length == 0) {
            ...
            G_SignalManager.dispatch(SignalConst.EVENT_TUTORIAL_STEP, "DrawOneEffect");
    ...
    _playAvatarOpen(rootNode, hero) {
        ...
        var effect = G_EffectGfxMgr.createPlayMovingGfx(rootNode, 'moving_card_open_yes', effectFunction.bind(this), handler(this, eventFunction), false);
    ...
    _createActionNode(effect) {
        if (effect == "kabaoa_b") {
            ...
            node.addComponent(cc.Sprite);
            UIHelper.loadTexture(node.getComponent(cc.Sprite), Path.getDrawCard('zhaomu_zi'))
            return node;
        }
            ...
            // --end
            return this._createHeroCardNode()
        }
        else if (effect == "kabaoa_b") {
            let node = new cc.Node();
            node.addComponent(cc.Sprite);
            UIHelper.loadTexture(node.getComponent(cc.Sprite), Path.getDrawCard('zhaomu_zi'))
            return node;
    ...
    play() {
        ...
        G_EffectGfxMgr.createPlayMovingGfx(this._nodeEffect, 'moving_draw_hero_kabaob', this.effectFunction.bind(this), handler(this, this.eventFunction), false);
    }
