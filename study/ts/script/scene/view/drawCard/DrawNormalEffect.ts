...
@ccclass
export default class DrawNormalEffect extends PopupBase {
    ...
    initData(awards: any, type?: any) {
        ...
        if (type == DrawNormalEffect.DRAW_TYPE_MONEY) {
        ...
        } else if (type == DrawNormalEffect.DRAW_TYPE_GOLD) {
    ...
    onFinishTouch(sender, event) {
        ...
        if (this._cardToOpen.length == 0) {
            ...
            G_SignalManager.dispatch(SignalConst.EVENT_TUTORIAL_STEP, "DrawNormalEffect");
    ...
    _createActionNode(effect: string) {
        if (effect == "kabaoa_a") {
            ...
            node.addComponent(cc.Sprite);
            UIHelper.loadTexture(node.getComponent(cc.Sprite), Path.getDrawCard('zhaomu_lan'))
            return node;
        }
        ...
        else if (effect == "card_to_hero_only_one") {
            //     --local index = 1
    ...
    play() {
        ...
        G_EffectGfxMgr.createPlayMovingGfx(this._nodeEffect, 'moving_draw_hero_kabaoa', this.effectFunction.bind(this), handler(this, this.eventFunction), false);
    }
