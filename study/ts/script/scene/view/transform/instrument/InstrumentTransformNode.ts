...
@ccclass
export default class InstrumentTransformNode extends cc.Component {
    @property({ type: CommonInstrumentAvatar, visible: true })
    _fileNodeItem: CommonInstrumentAvatar = null;
    ...
    _initView() {
        ...
        this._textTip.string = (Lang.get('transform_choose_tip' + this._type, { name: Lang.get('transform_tab_icon_3') }));
    }
    _resetView() {
    ...
    updateUI() {
        ...
        if (this._itemId > 0) {
            this._fileNodeItem.updateUI(this._itemId, this._limitLevel);
        ...
        if (this._itemCount > 1) {
            this._textNum.string = (Lang.get('transform_choose_count', {
                name: Lang.get('transform_tab_icon_3'),
