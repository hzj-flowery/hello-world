...
@ccclass
export default class CommonHeroName extends cc.Component {
    ...
    private _convertType: number = null;
    ...
    setName(baseId, rankLevel?, limitLevel?, outLine?, limitRedLevel?) {
        if (baseId == null) {
            this.node.active = (false);
            return;
        }
        if (this._convertType == null) {
            this._convertType = TypeConvertHelper.TYPE_HERO;
        }
        var heroParam = TypeConvertHelper.convert(this._convertType, baseId, null, null, limitLevel, limitRedLevel);
