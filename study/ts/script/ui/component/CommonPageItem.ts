...
@ccclass
export default class CommonPageItem extends ViewBase {
    ...
    _init() {
        ...
        this._page_View.node.on(cc.Node.EventType.TOUCH_START, handler(this, this._onPageTouch));
    ...
    setUserData(userData, selectPos) {
        ...
        this._page_View.setCurrentPageIndex(this._selectedPos - 1);
    }
    setCallBack(params) {
    ...
    _updateHeroAvatar(widget, index) {
        ...
        var commonHeroAvatar = cc.resources.get(Path.getCommonPrefab("CommonHeroAvatar"))
        var heroBaseId = cfgData.id;
        var node = cc.instantiate(commonHeroAvatar) as cc.Node;
    ...
    _showNearItem(show) {
        ...
        if (leftItem) {
            leftItem.active = true||(show);
            leftItem.opacity = 0;
        }
        if (rightItem) {
            rightItem.active = true||(show);
            rightItem.opacity = 0;
        }
    }
    _updateInfo() {
